...
class DataMonitor(Ui_MainWindow, QtGui.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.queue = Queue()
        self.datastream = DataStream(self.queue, (HOST, PORT), None, True)
        self.datastreamProcess = Process(target=self.datastream.serve_forever)
            ...
            for item in new_data:
                if item['TAG'] not in self.tags.keys():
                    self.tags[item['TAG']] = TagDataCurve(item['TAG'], 
                                                          ...
                                                          self.lqiLayout, 
                                                          len(self.tags.keys()))
                self.tags[item['TAG']].append(int(item['TIME']), float(item['T']), int(item['LQI']))
            self.max_temp=0
